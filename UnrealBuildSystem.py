...
if ST3:
    from UnrealScriptIDE.UnrealScriptIDEData import print_to_panel
...
def show_quick_panel(options, done):
    if ST3:
        # ST3 requires the list to have equal length, so fill lists that are too small with empty string.
        max_len = len(max(options, key=len))
        try:
            options = [x + [""]*(max_len-len(x)) if len(x) < max_len else x for x in options]
        except TypeError:
            print("error at: ", options)

        sublime.set_timeout(lambda: sublime.active_window().show_quick_panel(options, done), 0)
    else:
        sublime.active_window().show_quick_panel(options, done)
...
class UnrealBuildProjectCommand(sublime_plugin.TextCommand):
    ...
    def start_build(self):
        if self.settings.get('save_all_on_build'):
    ...
    def show_compile_options(self):
        ...
        # self.view.window().
        show_quick_panel(self.input_list, self.on_done_chose_compile_setting)
    ...
    def handle_thread(self, i=0, dir=1):
        ...
        # when finished, show output. b_build_and_run is set to true, just start last opened map:
            ...
            # == there were only warnings, ask what to do
                ...
                # self.view.window().
                show_quick_panel(input_list, self.on_done_warnings_input)
    ...
    def run_game(self):
        ...
        # self.view.window().
        show_quick_panel(input_list, self.on_done_run_game_input)
    ...
    def on_done_run_game_input(self, index):
        ...
        if index == 0:
            ...
            self.input_list = [
                ["Start with last used Configuration",
                 self.last_used_configuration],
            ...
            # self.view.window().
            show_quick_panel(self.input_list, self.on_done_chose_configuration)
    ...
    def edit_configurations(self):
        ...
        # self.view.window().
        show_quick_panel(input_list, self.on_done_edit_configurations)
    ...
    def on_done_edit_configurations(self, index):
        if index == 0:
            ...
            for config in self.startup_configurations:
                self.input_list.append([config] + self.startup_configurations[config])
            # self.view.window().
            show_quick_panel(self.input_list, self.on_done_edit_configuration)
        elif index == 2:
            for config in self.startup_configurations:
                self.input_list.append([config] + self.startup_configurations[config])
            # self.view.window().
            show_quick_panel(self.input_list, self.on_done_remove_configuration)
        elif index == -1:
            # escape, go back.
    ...
    def on_done_enter_name(self, name):
        if isinstance(name, basestring):
            ...
            # self.view.window().
            show_quick_panel(input_list, self.on_done_enter_name)
        else:
            if name == -1:
                self.edit_configurations()
            else:
    ...
    def on_done_enter_configuration(self, config):
        ...
        # self.view.window().
        show_quick_panel(input_list, self.on_done_entered_configuration)
    ...
    def on_done_edit_configuration(self, index):
        if index == -1:
            ...
            # self.view.window().
            show_quick_panel(config, self.on_done_edit_config_item)
        self.input_list = []
